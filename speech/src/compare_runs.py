...
def load_keras_model(path: str):
    # We only need forward passes for inference, so avoid compiling.
    # This sidesteps any unknown custom loss (e.g., "combo_loss") saved in older .h5 files.
    def batch_CCC(y_true, y_pred):  # kept for backward-compat model graphs
        return uf.CCC(y_true, y_pred)

    return load_model(
        path,
        custom_objects={'CCC': uf.CCC, 'batch_CCC': batch_CCC},
        compile=False
    )
...
print(f"[info] evaluating {len(ckpts)} checkpoint(s) × {len(VARIANTS)} variant(s)")
print(f"[out]  {RUN_DIR}")
